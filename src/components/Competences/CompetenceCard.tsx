...
const CompetenceCard = ({ competence, index }: { competence: (typeof competencesJsonData)[0]; index: number }) => {
  ...
  return (
    <div className="relative">
      <div
        className={`flex items-center justify-center gap-3 p-3 rounded-lg border-2 border-gray-200 bg-white transition-all duration-300 cursor-pointer hover:border-transparent hover:shadow-lg hover:scale-105`}
        style={{
      ...
      >
        <div className="flex items-center gap-1">
          <div className="w-6 h-6 flex items-center justify-center">
            <i className={`${competence.firstIcon} text-xl`} style={{ color: colorHex }}></i>
          </div>
          {competence.secondIcon && (
            <>
              <span className="">|</span>
              <div className="w-6 h-6 flex items-center justify-center">
                <i className={`${competence.secondIcon} text-xl`} style={{ color: secondColorHex }}></i>
              </div>
            </>
          )}
        </div>
        <span className="text-sm font-medium whitespace-nowrap">{competence.label}</span>
      </div>
